...
export default function CustomPathGenerator() {
  ...
  // Fetch all concepts and calculate concept type progress
  useEffect(() => {
    const fetchConceptsAndProgress = async () => {
      try {
        const concepts = await apiService.getAllConcepts();
        setAllConcepts(concepts);
        if (user) {
          const progress = await apiService.getUserProgress(user._id);
          // Group concepts by conceptType
          const typeMap: Record<string, { type: string; total: number; completed: number; masterySum: number }> = {};
          concepts.forEach((concept) => {
            const type = (concept as any)['conceptType'] || (concept as any)['Concept_Type'] || 'Uncategorized';
            if (!typeMap[type]) {
              typeMap[type] = {
                type,
                total: 0,
            ...
            const userConcept = progress.find((p) => p.conceptId === concept._id);
            const mastery = userConcept ? userConcept.score : 0;
            if (mastery >= 0.7) typeMap[type].completed += 1;
            typeMap[type].masterySum += mastery * 10; // 0-10 scale
          });
          // Convert to array and calculate mastery score
          const typeArr = Object.values(typeMap).map((t) => ({
            ...t,
            masteryScore: t.total > 0 ? t.masterySum / t.total : 0
          }));
          setCourseProgress(typeArr);
        }
      } catch (error) {
        console.error('Error fetching concepts or progress:', error);
      }
    };
    fetchConceptsAndProgress();
  }, [user]);
          ...
          <div className="lg:col-span-2 space-y-4 overflow-y-auto">
            ...
            <Card className="dark:bg-gray-800/80 dark:border-gray-700 shadow-lg">
              ...
              <CardContent className="space-y-4">
                {courseProgress.filter(type => type.masteryScore > 0).length === 0 ? (
                  <div className="text-gray-500 dark:text-gray-400">No progress data available.</div>
                ) : (
                  courseProgress
                    .filter(type => type.masteryScore > 0)
                    .slice(0, 8)
                    .map((type, idx) => (
                      <div key={type.type + idx} className="space-y-2">
                        ...
                        <Progress value={(type.completed / type.total) * 100} className="h-2" />
                        <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                          <span>Mastery: {type.masteryScore.toFixed(1)}/10</span>
                          <span>{Math.round((type.completed / type.total) * 100)}% Complete</span>
                        </div>
                      </div>
                    ))
                )}
                  ...
                  <div className="sticky bottom-0 bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm border-t pt-4 mt-8">
                    <div className="flex items-center justify-center space-x-3">
                      {getNextIncompleteTopic() ? (
                        ...
                        >
                          <Link
                            href={`/courses/${generatedPath[0]?.courseId}/concepts/${generatedPath[0]?.id}`}
                          >
                            <span className="flex items-center">
